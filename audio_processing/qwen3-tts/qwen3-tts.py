...
from model_utils import check_and_download_models, check_and_download_files  # noqa: E402
...
parser = get_base_parser( 'QWEN3 TTS', INPUT_WAV_PATH, SAVE_WAV_PATH)
# overwrite
parser.add_argument(
...
# ==========================================================================
# このファイルは qwen3-tts.py への差し替えパッチです。
# 以下の3ブロックをそれぞれ元ファイルの対応箇所と置き換えてください。
# ==========================================================================
...
class Qwen3TTS:
    ...
    def generate_icl_prompt(self, text_id, ref_id, ref_code, tts_pad_embed, tts_eos_embed):
        ...
        print(f"[ICL] trailing_text_hidden shape={trailing_text_hidden.shape}, "
              f"mean={trailing_text_hidden.mean():.6f}")
    ...
    def predict(self, text, ref_audio=None, ref_text=None):
        ...
        print(f"\n--- [DEBUG SYNC FINAL] ---")
        print(f"ROLE Mean: {role_proj.mean():.8f}")
        print(f"TAGS (inc SpkVec) Mean: {tags_combined.mean():.8f}")
        print(f"ICL Mean: {icl_input_embed.mean():.8f}")
        print(f"Final Prefill Input Shape: {talker_input_embed.shape}")
        print(f"Final Prefill Input Mean: {talker_input_embed.mean():.8f}")
        ...
        print(f"\n--- [AILIA PREFILL CHECK 1] ---")
        print(f"inputs_embeds        Mean : {talker_input_embed.mean():.8f}")   # 期待: -0.00202800
        print(f"attention_mask shape      : {attn_mask.shape}")                 # [1,1,L,L]
        print(f"attention_mask (finite)   : mean={finite_vals.mean():.4f}  "    # 有限値(0.0)のmean
              f"count={len(finite_vals)} / total={attn_mask.size}")
        print(f"position_ids              : {position_ids[0, :5]} ...")          # 0,1,2,3,4,...

        last_hidden, kv_caches = self._run_talker_decoder(
            talker_input_embed.astype(np.float32), attn_mask, position_ids, kv_caches
        )

        print(f"\n--- [AILIA PREFILL CHECK 2] ---")
        print(f"last_hidden shape         : {last_hidden.shape}")
        print(f"last_hidden Mean          : {last_hidden.mean():.8f}")           # 期待: 0.01353470
        print(f"last_hidden[-1] Mean      : {last_hidden[0, -1, :].mean():.8f}") # 最後トークンのhidden
        print(f"KV cache[0] (layer0 K)    : shape={kv_caches[0].shape}  "
              f"mean={kv_caches[0].mean():.8f}")                                 # 期待: -0.00154053
        print(f"KV cache[1] (layer0 V)    : shape={kv_caches[1].shape}  "
              f"mean={kv_caches[1].mean():.8f}")                                 # 期待: -0.00110607
        ...
        print(f"\n--- [AILIA PREFILL CHECK 3] ---")
        print(f"logits shape              : {logits.shape}")
        print(f"logits (last pos) min/max : {logits[0,-1,:].min():.4f} / {logits[0,-1,:].max():.4f}")
        print(f"logits Mean               : {logits.mean():.8f}")
        top5_ids   = np.argsort(logits[0, -1, :])[-5:][::-1]
        top5_probs = np.exp(logits[0, -1, top5_ids]) / np.sum(np.exp(logits[0, -1, :]))
        print(f"top-5 token ids           : {top5_ids}")
        print(f"top-5 probs               : {top5_probs.round(4)}")
        ...
        for step in range(MAX_NEW_TOKENS):
            ...
            if step < 5:
                print(f"\n[AILIA DECODE step={step}]")
                print(f"  group0             = {curr_token_id}"
                      f"  (PT: {[1751,1841,100,859,2][step]})")
                print(f"  text_feedback mean = {text_feedback.mean():.8f}"
                      f"  (PT: -0.00194550)")
                print(f"  inputs_embeds mean = {current_input.mean():.8f}"
                      f"  (PT: {[-0.00570679,-0.00491333,-0.00527954,-0.00138092,-0.00555420][step]:.8f})")
        ...
        print(f"[DECODE] Generated {len(generated_ids)} codec tokens")
...
def main():
    ...
    for f in file_list:
        check_and_download_files(f, REMOTE_PATH) 
    ...
    waveform = tts_engine.predict(text, ref_audio=ref_audio, ref_text=ref_text)
    
    # 4. 保存
    sf.write("ailia_result.wav", waveform.squeeze(), 24000)
    print("Saved as ailia_result.wav. Please compare with original output!")
